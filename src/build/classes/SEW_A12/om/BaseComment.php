...
abstract class BaseComment extends BaseObject implements Persistent
{
    ...
    public function getPosttime($format = 'Y-m-d H:i:s')
    {
        ...
        if ($this->posttime === '0000-00-00 00:00:00') {
        ...
        try {
            $dt = new DateTime($this->posttime);
        } catch (Exception $x) {
            throw new PropelException("Internally stored date/time/timestamp value could not be converted to DateTime: " . var_export($this->posttime, true), $x);
        }
    ...
    public function setPosttime($v)
    {
        ...
        if ($this->posttime !== null || $dt !== null) {
            $currentDateAsString = ($this->posttime !== null && $tmpDt = new DateTime($this->posttime)) ? $tmpDt->format('Y-m-d H:i:s') : null;
            $newDateAsString = $dt ? $dt->format('Y-m-d H:i:s') : null;
            if ($currentDateAsString !== $newDateAsString) {
                $this->posttime = $newDateAsString;
                $this->modifiedColumns[] = CommentPeer::POSTTIME;
            }
    ...
    public function setUsername($v)
    {
        ...
        if ($this->username !== $v) {
            $this->username = $v;
            $this->modifiedColumns[] = CommentPeer::USERNAME;
        }
    ...
    public function setEventid($v)
    {
        ...
        if ($this->eventid !== $v) {
            $this->eventid = $v;
            $this->modifiedColumns[] = CommentPeer::EVENTID;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false)
    {
        try {
            ...
            $this->posttime = ($row[$startcol + 2] !== null) ? (string) $row[$startcol + 2] : null;
            $this->username = ($row[$startcol + 3] !== null) ? (string) $row[$startcol + 3] : null;
            $this->eventid = ($row[$startcol + 4] !== null) ? (int) $row[$startcol + 4] : null;
            $this->resetModified();
    ...
    protected function doInsert(PropelPDO $con)
    {
        ...
        if ($this->isColumnModified(CommentPeer::POSTTIME)) {
            $modifiedColumns[':p' . $index++]  = '`postTime`';
        }
        if ($this->isColumnModified(CommentPeer::USERNAME)) {
            $modifiedColumns[':p' . $index++]  = '`username`';
        }
        if ($this->isColumnModified(CommentPeer::EVENTID)) {
            ...
            foreach ($modifiedColumns as $identifier => $columnName) {
                switch ($columnName) {
                    ...
                    case '`content`':
                        ...
                        break;
                    case '`postTime`':
                        $stmt->bindValue($identifier, $this->posttime, PDO::PARAM_STR);
                        break;
                    case '`username`':
    ...
    public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
    {
        ...
        if (array_key_exists($keys[2], $arr)) $this->setPosttime($arr[$keys[2]]);
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(CommentPeer::POSTTIME)) $criteria->add(CommentPeer::POSTTIME, $this->posttime);
        if ($this->isColumnModified(CommentPeer::USERNAME)) $criteria->add(CommentPeer::USERNAME, $this->username);
        if ($this->isColumnModified(CommentPeer::EVENTID)) $criteria->add(CommentPeer::EVENTID, $this->eventid);
